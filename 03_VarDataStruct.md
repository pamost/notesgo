...
### Логический тип
Ключевое слово bool используется для определения переменной логического типа. Существует два логических значения,
true или false, false является значением по умолчанию. В отличие от других языков, Go не позволяет нам конвертировать
булево значение в число.

```
// пример
var isActive bool // глобальная переменная
var enabled, disabled = true, false // тип переменных bool
func test() {
